...
function pointInTriangle(point, a, b, c) {
    const d1 = cross2D(point.screenX, point.screenY, a.screenX, a.screenY, b.screenX, b.screenY);
    const d2 = cross2D(point.screenX, point.screenY, b.screenX, b.screenY, c.screenX, c.screenY);
    const d3 = cross2D(point.screenX, point.screenY, c.screenX, c.screenY, a.screenX, a.screenY);
    const hasNegative = d1 < -1e-7 || d2 < -1e-7 || d3 < -1e-7;
    const hasPositive = d1 > 1e-7 || d2 > 1e-7 || d3 > 1e-7;
    return !(hasNegative && hasPositive);
}
function cleanFaceIndices(face, projected) {
    const result = [];
    for (const rawIndex of face.indices) {
        const index = Math.abs(rawIndex);
        if (projected[index] === undefined)
            continue;
...
function polygonArea(indices, projected) {
    let area = 0;
    for (let index = 0; index < indices.length; index++) {
        const current = projected[indices[index]];
        const next = projected[indices[(index + 1) % indices.length]];
        area += current.screenX * next.screenY - next.screenX * current.screenY;
    }
    return area / 2;
}
function triangulateFace(face, projected) {
    ...
    while (working.length > 3 && guard-- > 0) {
        ...
        for (let i = 0; i < working.length; i++) {
            ...
            const convexity = cross2D(previous.screenX, previous.screenY, current.screenX, current.screenY, next.screenX, next.screenY);
            if (convexity * orientation <= 1e-7)
                continue;
    ...
    if (triangles.length !== indices.length - 2) {
        ...
        for (let i = 1; i < indices.length - 1; i++) {
            ...
            const b = projected[indices[i]];
            const c = projected[indices[i + 1]];
            if (Math.abs(cross2D(a.screenX, a.screenY, b.screenX, b.screenY, c.screenX, c.screenY)) > 1e-7) {
...
function faceShade(indices, projected) {
    if (indices.length < 3)
        return [92, 118, 148];
    let normal = new Point3D(0, 0, 1);
    for (let i = 1; i < indices.length - 1; i++) {
        const a = projected[indices[0]].eye;
        const b = projected[indices[i]].eye;
        const c = projected[indices[i + 1]].eye;
...
export class SoftwareRenderer {
    ...
    render(context, canvas, model, mode) {
    ...
    collectTriangles(faces, projection) {
        const triangles = [];
    ...
    drawWireframe(context, edges, projection) {
    ...
    rasterizeDepth(triangles, projection, pixels) {
        ...
        for (const triangle of triangles) {
            ...
            if (a === undefined || b === undefined || c === undefined)
                continue;
            const area = cross2D(a.screenX, a.screenY, b.screenX, b.screenY, c.screenX, c.screenY);
            ...
            for (let y = minY; y <= maxY; y++) {
                ...
                for (let x = minX; x <= maxX; x++) {
                    const sampleX = x + 0.5;
                    const wA = cross2D(b.screenX, b.screenY, c.screenX, c.screenY, sampleX, sampleY) / area;
                    const wB = cross2D(c.screenX, c.screenY, a.screenX, a.screenY, sampleX, sampleY) / area;
                    const wC = 1 - wA - wB;
                    if (wA < -1e-5 || wB < -1e-5 || wC < -1e-5)
                        continue;
                    const invDepth = wA * a.invDepth +
                        wB * b.invDepth +
                        wC * c.invDepth;
                    const bufferIndex = y * width + x;
                    if (invDepth <= this.depthBuffer[bufferIndex])
                        continue;
    ...
    maxNeighborDepth(x, y) {
        let maximum = FAR_VALUE;
        for (let offsetY = -1; offsetY <= 1; offsetY++) {
            const sampleY = y + offsetY;
            if (sampleY < 0 || sampleY >= this.bufferHeight)
                continue;
            for (let offsetX = -1; offsetX <= 1; offsetX++) {
                const sampleX = x + offsetX;
                if (sampleX < 0 || sampleX >= this.bufferWidth)
                    continue;
                maximum = Math.max(maximum, this.depthBuffer[sampleY * this.bufferWidth + sampleX]);
            }
        }
        return maximum;
    }
    drawDepthTestedEdges(context, edges, projection, strokeStyle, lineWidth) {
        ...
        for (const edge of edges) {
            ...
            for (let step = 0; step <= steps; step++) {
                ...
                const visible = surfaceInvDepth === FAR_VALUE ||
