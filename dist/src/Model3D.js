...
export class Model3D {
    constructor() {
        ...
        this.faces = [];
        this.pieces = [];
        this.indexToPiece = new Map();
        this.pieceTransforms = [];
        ...
        this.radius = 1;
        this.sourceName = 'Sin modelo';
    }
    loadFromText(text, sourceName = 'Modelo cargado') {
        ...
        const definedVertices = vertices.filter((vertex) => vertex !== undefined);
        const min = new Point3D(Math.min(...definedVertices.map((vertex) => vertex.x)), Math.min(...definedVertices.map((vertex) => vertex.y)), Math.min(...definedVertices.map((vertex) => vertex.z)));
        const max = new Point3D(Math.max(...definedVertices.map((vertex) => vertex.x)), Math.max(...definedVertices.map((vertex) => vertex.y)), Math.max(...definedVertices.map((vertex) => vertex.z)));
        const center = scale(add(min, max), 0.5);
        this.baseVertices = vertices.map((vertex) => (vertex === undefined ? undefined : subtract(vertex, center)));
        this.faces = parsedFaces;
        this.radius = Math.max(1, ...this.baseVertices
    ...
    detectPieces() {
        ...
        if ((components.length === 1 || components.length > 12) &&
    ...
    rotatePiece(pieceIndex, axis, radians) {
        const transform = this.pieceTransforms[pieceIndex];
        if (!transform)
            return;
    ...
    resetPiece(pieceIndex) {
        if (!this.pieceTransforms[pieceIndex])
            return;
    ...
    zoomCamera(factor) {
        this.camera.rho = clamp(this.camera.rho * factor, this.radius * 1.25, this.radius * 80);
    }
    transformedVertex(index) {
    ...
    project(width, height) {
        const projected = new Array(this.baseVertices.length);
        ...
        for (let index = 1; index < this.baseVertices.length; index++) {
            ...
            const eyeY = -cosPhi * cosTheta * world.x -
                cosPhi * sinTheta * world.y +
                sinPhi * world.z;
            const eyeZ = sinPhi * cosTheta * world.x +
